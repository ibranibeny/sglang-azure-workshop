...
#   - SGLang container serving the model (default Qwen3.6-35B-A3B-FP8) on a
#     single H100 NVL (94GB) GPU, bound to loopback only (never exposed directly).
...
#
# NOTE: the SGLang launch flags below are tuned for the hybrid Qwen3.6-35B-A3B
# model (Mamba2 + MoE + MTP) — "Profile A" in references/model-catalog.md. For a
# different model family, swap the flags per that catalog (drop the --mamba-* /
# --speculative-* flags and SGLANG_ENABLE_SPEC_V2, and set the right parsers).
# ---------------------------------------------------------------------------
set -euo pipefail
cd "$(dirname "$0")"
source ./config.sh
...
# (re-running this script) restarts only Caddy and does NOT reload the model.
